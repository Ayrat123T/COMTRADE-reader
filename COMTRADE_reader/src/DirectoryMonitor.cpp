        ...
        if (status.type() == std::filesystem::file_type::directory) {
            ...
            for (const auto& directory : directories) {
                if (directory.second.type() != std::filesystem::file_type::directory
                    && (std::get<0>(directory).extension().string().find(".cff"sv) != -1
                    || std::get<0>(directory).extension().string().find(".cfg"sv) != -1
                    || std::get<0>(directory).extension().string().find(".dat"sv) != -1)) {
                    CreateCopyOfPath(std::get<0>(directory));
                }
                FindComtradeFiles(std::get<0>(directory), std::get<1>(directory));
            }
        }
    }
        ...
        if (status.type() == std::filesystem::file_type::directory) {
            ...
            for (const auto& directory : directories) {
                if (directory.second.type() != std::filesystem::file_type::directory
                    && (std::get<0>(directory).extension().string().find(".cff"sv) != -1
                    || std::get<0>(directory).extension().string().find(".cfg"sv) != -1)) {
                    comtrade_files_.push_back(std::get<0>(directory));
                }
                SetComtradeFiles(std::get<0>(directory), std::get<1>(directory));
            }
        }
    }
