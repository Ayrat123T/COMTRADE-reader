...
void TestSinglelaunch(const filesystem::path& comtrade_files_path) {
    ...
    vector<comtrade::Comtrade> comtrades = Comtrade_IO::ReadComtrades(directory_monitor.GetComtradeFilesPath());
    directory_monitor.PrintComtradeFilesNames(cout);
    /* расчёты */ 
...
void TestSinglelaunchWithCreateArchive(const filesystem::path& comtrade_files_path) {
    DirectoryMonitor::DirectoryMonitor directory_monitor(filesystem::path{ comtrade_files_path });
    directory_monitor.PrintTree(cout);
    directory_monitor.CreateArchive(nullopt);
    vector<comtrade::Comtrade> comtrades = Comtrade_IO::ReadComtrades(directory_monitor.GetComtradeFilesPath());
    directory_monitor.PrintComtradeFilesNames(cout);
    /* расчёты */ 
    for (auto comtrade : comtrades) {
...
int main(int argc, char*argv[]) {
    ...
    const filesystem::path comtrade_files_path = path(buffer).parent_path().parent_path() / "oscillograms"s; //Получить деректорию с COMTRADE файлами
